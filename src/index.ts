...
class ParallaxX {
  ...
  private getPxxx(element: HTMLElement): PxxParams {
    ...
    const parseTranslate = (value: string): Pxx => {
      const isPreset = Object.values(TranslatePreset).includes(value as TranslatePreset)
    ...
    const parseOpacity = (value: string): Pxx => {
      const isPreset = Object.values(OpacityPreset).includes(value as OpacityPreset)
